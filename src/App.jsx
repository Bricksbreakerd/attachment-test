...
import { Clock, ArrowLeft, ArrowRight, CheckCircle, Activity, Brain, Shield, Download, BookOpen, Heart, Zap, Lock, DollarSign, MessageCircle, Loader2 } from 'lucide-react';
...
const rawQuestions = [
  ...
  { id: 12, category: "冲突应对与沟通模式", text: "伴侣指出你最近对这段关系不太上心（比如很少主动约会或关心ta），语气稍微有点重。", options: [{ label: "A", text: "我承认最近确实有点懈怠，解释一下原因，以后注意。", type: "A" }, { label: "B", text: "我感到很委屈，觉得ta是不是嫌弃我了，忍不住哭诉自己平时的付出。", type: "B" }, { label: "C", text: "我不爽地看着ta，心里想“你也没好到哪去”，然后不理ta走开。", type: "C" }, { label: "D", text: "我立刻反击，翻出ta以前的错事来回怼，不想让自己处于下风。", type: "D" }] },
  { id: 13, category: "冲突应对与沟通模式", text: "你们在讨论未来发展（如定居城市）时产生了分歧，伴侣态度很强硬。", options: [{ label: "A", text: "我尝试理解ta的理由，但也坚定表达我的想法，寻找折中方案。", type: "A" }, { label: "B", text: "我可能会妥协，心想“只要能在一起就行”，虽然心里挺委屈的。", type: "B" }, { label: "C", text: "觉得没法沟通，直接说“以后再说吧”，心里开始盘算如果不行的退路。", type: "C" }, { label: "D", text: "我情绪比较激动，指责ta自私，甚至气头上会说“那不如分手”。", type: "D" }] },
  ...
  { id: 26, category: "分离、独立与个人空间", text: "为了睡眠质量，伴侣提议偶尔分开睡（比如分房或各回各家）。", options: [{ label: "A", text: "如果能睡得更好，我愿意尝试，不代表感情不好。", type: "A" }, { label: "B", text: "坚决反对，觉得这是分手的征兆，不抱着睡不踏实。", type: "B" }, { label: "C", text: "立刻同意，其实我早想这么提了，自己睡舒服多了。", type: "C" }, { label: "D", text: "觉得ta是不是嫌弃我，或者为了方便偷偷玩手机。", type: "D" }] },
  ...
  { id: 33, category: "压力情境与支持寻求", text: "伴侣因一时冲动造成了不小的经济损失，需要共同面对。", options: [{ label: "A", text: "虽然生气，但一起坐下来算账，制定计划共同承担。", type: "A" }, { label: "B", text: "非常恐慌，觉得安全感没了，不停指责ta，甚至想分手。", type: "B" }, { label: "C", text: "会帮忙，但会划清界限，“这是你的错，你自己解决”。", type: "C" }, { label: "D", text: "感到绝望，觉得这验证了“谁都靠不住”的想法。", type: "D" }] },
  ...
  // 46-50: 财务
  { id: 46, category: "财务管理与资源共享", text: "谈到约会花销或未来金钱管理时。", options: [{ label: "A", text: "主张根据能力公平分担，有些可以共同支付，有些独立。", type: "A" }, { label: "B", text: "希望钱不分你我，这样才觉得是一家人，分太清不好。", type: "B" }, { label: "C", text: "坚持AA制或分得越清越好，不想有金钱纠葛。", type: "C" }, { label: "D", text: "随便，但总担心ta卷钱跑了或用钱控制我。", type: "D" }] },
  ...
  { id: 49, category: "财务管理与资源共享", text: "伴侣提出一个需要深度绑定的长期计划（比如买房、移居或领养宠物）。", options: [{ label: "A", text: "这是重大承诺，需要详细评估和规划。", type: "A" }, { label: "B", text: "很开心，觉得终于锁住ta了，这是关系的纽带。", type: "B" }, { label: "C", text: "想到要被长期绑在一起，就本能地想逃跑。", type: "C" }, { label: "D", text: "担心万一分手怎么分，会变成噩梦。", type: "D" }] },
  ...
  <div className={`flex flex-col items-center justify-center min-h-screen ${morandi.bg} p-4 sm:p-6 text-center`}>
    <div className={`${morandi.card} p-6 sm:p-10 rounded-3xl shadow-xl max-w-2xl w-full`}>
      ...
      <div className="bg-[#F9F9F9] p-4 sm:p-6 rounded-xl mb-6 sm:mb-10 text-left text-xs sm:text-sm text-gray-600 space-y-2">
        ...
        <p>💌 <strong>单身/恋爱中：</strong> 题目适用于所有阶段。如果您目前单身，请代入一段想象中的亲密关系。</p>
...
const ResultScreen = ({ answers, timeData }) => {
  ...
  // --- 图片导出处理 (使用 html2canvas CDN) ---
  // 我们动态加载 CDN 脚本，避免本地环境依赖问题
  useEffect(() => {
  ...
  const handleExportImage = () => {
    ...
    if (typeof window !== 'undefined' && window.html2canvas && resultRef.current) {
      setIsExporting(true);
      // 调用全局的 html2canvas
      window.html2canvas(resultRef.current, {
        backgroundColor: '#F0F2F0', // 确保背景色正确
        scale: 2, // 高清截图
        useCORS: true
      }).then((canvas) => {
  ...
  return (
    <div className={`min-h-screen ${morandi.bg} p-4 sm:p-10 pb-20`}>
      ...
      <div ref={resultRef} className={`max-w-5xl mx-auto space-y-8 ${morandi.bg} p-4`}>
        ...
        {/* 头部结果卡片 */}
        <div className={`${morandi.card} rounded-3xl p-6 sm:p-10 shadow-lg text-center relative overflow-hidden`}>
        ...
        {/* 维度分析雷达 */}
        <div className={`${morandi.card} rounded-3xl p-6 sm:p-8 shadow-lg flex flex-col items-center`}>
        ...
        {/* 心理处方 (新增) */}
        <div className={`${morandi.card} rounded-3xl p-6 sm:p-8 shadow-lg bg-[#F2F7F6]`}>
      ...
      <button
        ...
        className="fixed bottom-8 right-8 bg-[#6A665D] text-white p-4 rounded-full shadow-2xl hover:bg-[#4a4741] transition-all no-print z-50 flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
        title="保存为图片"
      >
        {isExporting ? <Loader2 className="w-6 h-6 animate-spin" /> : <Download className="w-6 h-6" />}
        <span className="text-sm font-medium">{isExporting ? '生成中...' : '保存为图片'}</span>
      </button>
    </div>
  );
};
